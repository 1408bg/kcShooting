...
function showAlert(message) {
  const alertKnot = new TextKnot({ x: canvas.width/2, y: 25, text: message, fontSize: 27, opacity: 0 });
...
function* moving() {
  const moveSpeed = 4;
  const delay = new Duration({ milisecond: 33 });
  while (true) {
...
function* shooting() {
  const shootCooldown = new Duration({ milisecond: 500 });
  let canShoot = true;
